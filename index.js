...
// Nested function is able to acces internal bacause it is in the parent ScopedCredential. This is called a closure.
...
const zooAnimals = [
  ...
  */
  function animalNames(data){
      let displayNames = [];
        let nameMaker = data.forEach(function(item){
          // console.log(item)
...
console.log(animalNames(zooAnimals));
  ...
  */
  function USApop(data){
    const fetcher = data.map(function(item){
          return item.population;
    })
    const population = fetcher.reduce(function(acc, item){
      return acc+item;
    },0)
    return population;
